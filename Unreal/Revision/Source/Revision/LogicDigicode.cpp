...
void ALogicDigicode::Tick(float DeltaTime)
{
	...
	if (CheckDigicode())
	{
		...
		GetWorld()->GetAuthGameMode<ARevisionGameModeBase>()->GetGameManager()->GetGameWidget()->SetNextEnabled(true);
		//GetWorld()->GetFirstPlayerController()->SetShowMouseCursor(true);
	}
	else
	{
		lamp->SetIsActive(false);
		GetWorld()->GetAuthGameMode<ARevisionGameModeBase>()->GetGameManager()->GetGameWidget()->SetNextEnabled(false);
